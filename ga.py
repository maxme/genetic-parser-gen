...
class GA:
    def __init__(self, data, expected_result, popsize = 50, mut_rate=20,
                 GA_printers = None):
        # FIXME: dirty & useless
    ...
    def run_once(self):
        ...
        for i in xrange(3, self.popsize):
            tmpchrom = Individual(mother=self.__my_choice(),
    ...
    def end(self, outfilename):
        ...
        print best.st()
        print "ratio generation/population_size: %.3f" %(self.generation \
              / float(POP_SIZE))
...
class GA_print_text:
    def refresh_screen(self, ga):
        print "Generation: %d (mutation rate=%d)" % (ga.generation,
                                                     Individual.mutation_rate)
        for i in ga.population[:5]:
            print i

def run(data, expected, outfilename):
    a = GA(data, expected, POP_SIZE, MUTATION_RATE,
